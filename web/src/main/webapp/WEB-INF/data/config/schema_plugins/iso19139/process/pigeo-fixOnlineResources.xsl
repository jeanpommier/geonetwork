...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  xmlns:gmd="http://www.isotc211.org/2005/gmd" version="2.0">
   ...
   <!-- Adds a description to DOWNLADABLE files, when missing -->
    ...
    <xsl:template match="gmd:CI_OnlineResource[contains(gmd:protocol/gco:CharacterString, 'WMS')]" priority="2">
    ...
    <!-- Fixes namespace-in-layer-name WMS resources pb when setup by Philippe : 
			moves the namespace prefix, when located in layer name, to the URL. Applies only to pigeo URLs
    ...
    <xsl:template match="gmd:CI_OnlineResource[contains(gmd:protocol/gco:CharacterString, 'WMS')]" priority="3">
    	<xsl:choose>
         <xsl:when test=".[contains(gmd:linkage/gmd:URL, 'pigeo') and contains(gmd:name/gco:CharacterString, ':')] ">
