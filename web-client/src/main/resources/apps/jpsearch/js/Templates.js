...
GeoNetwork.Templates.TITLE = '<h1 class="{format}"><input type="checkbox" <tpl if="selected==\'true\'">checked="true"</tpl> class="selector" onclick="javascript:catalogue.metadataSelect((this.checked?\'add\':\'remove\'), [\'{uuid}\']);"/><a href="#" onclick="javascript:catalogue.metadataShow(\'{uuid}\');">{title}</a>' +
    ...
    '<ul>',
        '<tpl for=".">',
            '<li class="md md-simple" title="{abstract}">',
                '<table class="icon"  style="border-color:{bboxColor}"><tr>',
/*                '<td style="width:30px;">',  // FIXME
                GeoNetwork.Templates.LOGO,
                '</td>',
*/                '<td id="{uuid}">',
                GeoNetwork.Templates.TITLE,
                '<span class="subject">'+OpenLayers.i18n("keywords")+' : <tpl for="subject">',
                    '{value}{[xindex==xcount?"":", "]}',
                '</tpl></span>',
                '</td></tr></table>',
            '</li>',
...
GeoNetwork.Templates.THUMBNAIL = new Ext.XTemplate(
        '<ul>',
            '<tpl for=".">',
                ...
                '<div class="md-wrap icon" id="{uuid}" title="{abstract}" style="border-color:{bboxColor}">',
                    GeoNetwork.Templates.TITLE,
					'<div class="thumbnail">',
	                    '<tpl if="thumbnail">',
	                ...
	                '</div>',
                    '<tpl if="this.hasHttpLinks(links)">',
						'<div class="md-mn md-mn-links links-http" title="http">&nbsp;</div>',
					'</tpl>',
					'<tpl if="this.hasDownloadLinks2(values.links)">',
						'<div class="md-mn md-mn-links links-download" title="download">&nbsp;</div>',
					'</tpl>',
					'<tpl if="this.hasWmsLinks(values.links)">',
						'<div class="md-mn md-mn-links links-wms" title="Web Map Service">&nbsp;</div>',
					'</tpl>',
					'<tpl if="this.hasGELinks(values.links)">',
						'<div class="md-mn md-mn-links links-ge" title="Google Earth">&nbsp;</div>',
					'</tpl>',
                '</div>',
                '</li>',
            '</tpl>',
        '</ul>',
        {
        	hasDownloadLinks2: function(values) {
	            var i;
...
GeoNetwork.Templates.FULL = new Ext.XTemplate(
        '<ul>',
          '<tpl for=".">',
            '<li class="md md-full"  title="{abstract}">',
                '<table><tr>',
                /*'<td class="left">',
                    GeoNetwork.Templates.LOGO,
                '</td>',*/
                '<td id="{uuid}">',
                     '<div class="thumb">',
                        ...
                        '<div class="thumbnail">',
                            '<tpl if="thumbnail">',
                            	'<tpl if="image">',
                                	'<a rel="lightbox" href="{image}"><img src="{thumbnail}" alt="Thumbnail"/></a>', 
                            	'</tpl>',
                            	'<tpl if="image==\'\'">',
                            		'<a rel="lightbox" href="{thumbnail}"><img src="{thumbnail}" alt="Thumbnail"/></a>', 
                        		'</tpl>',
                ...
                '</div>',
                GeoNetwork.Templates.TITLE,
/*                    '<p class="abstract">{[values.abstract.substring(0, 250)]} ...</p>',    */// FIXME : 250 as parameters
                    ...
                    // FIXME : ref to app
                        '<tpl for="links">',
                            '<tpl if="values.type == \'application/vnd.ogc.wms_xml\' || values.type == \'OGC:WMS\'">',
                                '<a href="#" class="md-mn addLayer" title="' + OpenLayers.i18n('addToMap') + ' {title}" alt="Add layer to map" onclick="app.getIMap().addWMSLayer([[\'{[values.name]}\', \'{href}\', \'{name}\', \'{id}\']]);">&nbsp;</a>',
                            '</tpl>',
...
*/              '<td class="icon"  style="border-color:{bboxColor}" >', 
                ...
                '<div class="md-contact">',
                  ...
                  '<tpl if="edit==\'true\' && isharvested!=\'y\'">',
                      '<span class="md-mn md-mn-user" title="' + OpenLayers.i18n('ownerName') + '">{ownername} (' + OpenLayers.i18n('lastUpdate') + '{[values.changedate.split(\'T\')[0]]})</span>',
                  '</tpl>',
                '</div>',
            '</li>',
        '</tpl>',
